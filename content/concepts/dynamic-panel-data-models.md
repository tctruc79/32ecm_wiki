...
Nhiều hiện tượng kinh tế không "quên" quá khứ — giá trị hôm nay phụ thuộc mạnh vào giá trị hôm qua. Hai ví dụ cụ thể:
<br><span class="en">Many economic phenomena do not "forget" the past — today's value depends strongly on yesterday's value. Two concrete examples:</span>
...
Có ba lý do (giữ nguyên thuật ngữ gốc) để đặc tả mô hình theo dạng động:
<br><span class="en">There are three reasons (original terms kept) for specifying a model in dynamic form:</span>
...
Dạng tổng quát nhất, dùng xuyên suốt các mục sau, có thêm một vector biến nội sinh khác $Y_{it}$ (ngoài $y_{i,t-1}$):
<br><span class="en">The most general form, used throughout the following sections, adds another vector of endogenous variables $Y_{it}$ (besides $y_{i,t-1}$):</span>
...
**Bộ dữ liệu minh họa** dùng xuyên suốt Lecture 8 (300 doanh nghiệp, quan sát 10 năm — dài hơn 5 năm ở Topic 12–13, vì mô hình động và GMM cần đủ độ dài chuỗi thời gian để có đủ lag làm instrument):
<br><span class="en">**Illustrative dataset** used throughout Lecture 8 (300 firms, observed over 10 years — longer than the 5 years in Topic 12–13, because dynamic models and GMM need enough time-series length to have sufficient lags to use as instruments):</span>
...
Đúc kết trực giác: instrument lag-level **thường mạnh nhưng dễ không hợp lệ** (nếu có tự tương quan); instrument lag-diff **thường yếu hơn nhưng dễ hợp lệ hơn**. Đây là một đánh đổi kinh điển giữa relevance và exogeneity trong chọn instrument.
<br><span class="en">The intuition in a nutshell: the lag-level instrument is **usually strong but easily invalid** (if serial correlation is present); the lag-diff instrument is **usually weaker but more easily valid**. This is a classic trade-off between relevance and exogeneity in instrument choice.</span>
...
**Sức mạnh (strength)**: $Cov(y_{i,t-1},\Delta y_{i,t-1})=Var(y_{i,t-1})-Cov(y_{i,t-1},y_{i,t-2})$ — mạnh khi quá trình **không phải unit-root** ($\rho\neq1$). Nếu $\rho=0$, quá trình không có tính động ($\alpha=0$, không có gì để bàn về instrument). Có thể dùng lag sâu hơn ($\Delta y_{i,t-2}$…) nhưng yếu dần; trong thực hành thường dừng ở $\Delta y_{i,t-1}$ — một lưu ý thực tế: hàm `pgmm()` trong R **không hỗ trợ** lag sâu hơn cho phương trình mức.
...
So với Difference GMM ($\hat\alpha=0.822$, SE$=0.021$), System GMM cho SE nhỏ hơn nữa (0.006) — đúng như kỳ vọng lý thuyết ở mục 6.1 (thêm điều kiện moment ở mức giúp tăng hiệu quả khi $y$ persistent). Nhưng đây cũng chính là dịp để **áp dụng trực tiếp 3 dấu hiệu gián tiếp kiểm tra tính dừng** được đề xuất (mục 7.5) vào chính bảng số này:
<br><span class="en">Compared to Difference GMM ($\hat\alpha=0.822$, SE$=0.021$), System GMM gives an even smaller SE (0.006) — exactly as the theoretical expectation in section 6.1 predicts (adding moment conditions at level increases efficiency when $y$ is persistent). But this is also the occasion to **directly apply the 3 indirect signs for checking stationarity** proposed in section 7.5 to this very table of results:</span>
...
2. **Kiểm tra $\hat\alpha$ không quá gần 0 hoặc 1**: $\hat\alpha_{SGMM}=0.9999$ — **cực kỳ gần 1** (gần như unit-root/random walk) — chính xác là loại giá trị cần cảnh giác.
<br><span class="en">**Check that $\hat\alpha$ is not too close to 0 or 1**: $\hat\alpha_{SGMM}=0.9999$ — **extremely close to 1** (near unit-root/random walk) — exactly the kind of value to be cautious about.</span>
3. **Vẽ đồ thị $y_{it}$ theo thời gian**: không có dữ liệu để vẽ trực tiếp tại đây, nhưng đây là bước thứ ba bắt buộc trong thực hành để xác nhận trực quan hành vi chuỗi.
<br><span class="en">**Plot $y_{it}$ over time**: there is no data to plot directly here, but this is the mandatory third step in practice to visually confirm the series' behavior.</span>

**Rút ra**: hai trong ba dấu hiệu gián tiếp, khi áp vào chính ví dụ này, đều gợi ý giả định tính dừng (điều kiện cho tính hợp lệ của instrument phương trình mức, mục 6.4–6.5) **đáng ngờ** — dù các kiểm định chính thức AR(1)/AR(2)/Sargan đều "đẹp" (pass) như thường lệ. Đây là quan sát rút ra bằng cách áp dụng đúng bộ tiêu chí nêu ở mục 7.5 vào chính bảng kết quả ở trên — không phải khẳng định mô hình "sai," mà là một lời nhắc thận trọng: **SE rất nhỏ + AR/Sargan pass không tự động đảm bảo tính dừng** — vì không có một kiểm định chính thức duy nhất cho tính dừng trong khung System GMM, ba dấu hiệu gián tiếp này luôn cần được đối chiếu cùng nhau, không dừng lại ở mỗi Sargan/AR "đẹp."
...
→ **Quan sát AR(1) có ý nghĩa thống kê (bị bác bỏ) chính là điều được kỳ vọng** — dấu hiệu mô hình đang hoạt động đúng như lý thuyết dự đoán. → Ngược lại: **nếu AR(1) không bác bỏ được**, đây mới là điều **bất thường**, gợi ý sai đặc tả mô hình hoặc lỗi dữ liệu. Minh họa từ ví dụ AB ở mục 4.4: AR(1) $z=-12.346$, $p\approx0$ → bác bỏ mạnh → đúng như kỳ vọng.
<br><span class="en">→ **Observing statistically significant AR(1) (rejected) is precisely what is expected** — a sign that the model is behaving exactly as theory predicts. → Conversely: **if AR(1) fails to be rejected**, this is the truly **abnormal** outcome, suggesting model misspecification or a data error. Illustration from the AB example in section 4.4: AR(1) $z=-12.346$, $p\approx0$ → strongly rejected → exactly as expected.</span>
...
1. Đặc tả mô hình động **để có cớ** dùng System GMM — đây là **sai quy trình**: đặc tả động phải xuất phát từ bản chất kinh tế, ước lượng chọn theo sau.
<br><span class="en">Specifying a dynamic model **as an excuse** to use System GMM — this is the **wrong procedure**: a dynamic specification must originate from the economic nature of the process, with the estimator chosen after.</span>

...
> **Ghi chú về nguồn (mâu thuẫn nội bộ trong slide)**: ở đúng slide trình bày phép tính "exact" cho log-log, công thức hiển thị dùng số mũ $\beta_1=0.297$ — tức $b=(1.01^{0.297}-1)\times100=0.296$ — **không khớp** với hệ số $\beta_{\ln schooling}=0.361$ mà chính slide vừa nêu ở dòng "approximate" ngay phía trên (và cũng là con số thực trong bảng hồi quy log-log ở trên: 0.36100). Điều kỳ lạ hơn: câu kết luận bằng lời ngay sau phép tính lại ghi "kết quả là 0.360 percent" — tức khớp chính xác với việc dùng $\beta_1=0.361$ (không phải 0.297) trong công thức. Nhiều khả năng đây là số liệu "0.297" sót lại từ một phiên bản hồi quy/bộ dữ liệu cũ mà slide quên cập nhật khi thay bằng dữ liệu/hệ số mới (0.361), trong khi câu kết luận bằng lời đã được cập nhật đúng. Trang wiki này ghi chú lại mâu thuẫn này thay vì âm thầm sửa gốc, và **dùng $\beta_1=0.361$ (khớp với bảng hồi quy thật) để tính lại phép "exact" ở trên cho đúng phương pháp**, thay vì lặp lại số 0.297 sai lệch.
<br><span class="en">**Source note (internal inconsistency in the slide)**: on the exact slide presenting the "exact" calculation for log-log, the formula shown uses the exponent $\beta_1=0.297$ — i.e. $b=(1.01^{0.297}-1)\times100=0.296$ — which **does not match** the coefficient $\beta_{\ln schooling}=0.361$ that the same slide just stated on the "approximate" line right above it (and which is also the actual figure in the log-log regression table above: 0.36100). Even stranger: the verbal conclusion right after the calculation states "the result is 0.360 percent" — which matches exactly if $\beta_1=0.361$ (not 0.297) is used in the formula. This "0.297" figure is most likely a leftover from an older regression/dataset version that the slide forgot to update when it switched to the new data/coefficients (0.361), while the verbal conclusion was updated correctly. This wiki page notes the inconsistency rather than silently correcting the source, and **uses $\beta_1=0.361$ (matching the actual regression table) to recompute the "exact" calculation above with the correct method**, instead of repeating the erroneous 0.297 figure.</span>
...
> **Ghi chú về nguồn (mâu thuẫn tương tự mục 3.2, xảy ra ở đúng ví dụ `schooling` continuous regressor của log-lin)**: slide viết "$b=(e^{0.297}-1)\times100=0.027$: kết quả là 2.700 percent" — số mũ $0.297$ ở đây **cũng không khớp** với $\beta_{schooling}=0.02667$ vừa nêu (và khớp với bảng hồi quy log-lin thật ở trên: 0.026674), lại còn thiếu bước nhân 100 khi hiển thị số trung gian ($0.027$ thay vì $2.7$). Nhưng câu kết luận cuối "2.700 percent" một lần nữa khớp chính xác nếu dùng $\beta_1=0.02667$ (không phải 0.297). Cùng một con số "0.297" xuất hiện lặp lại ở cả hai chỗ (log-log lẫn log-lin) càng củng cố khả năng đây là dữ liệu cũ sót lại từ một phiên bản trước của slide/bộ dữ liệu, không phải hai lỗi độc lập ngẫu nhiên. Ghi chú lại ở đây, không âm thầm sửa vào nguồn.
<br><span class="en">**Source note (similar inconsistency to section 3.2, occurring in exactly the `schooling` continuous-regressor example for log-lin)**: the slide writes "$b=(e^{0.297}-1)\times100=0.027$: the result is 2.700 percent" — the exponent $0.297$ here **also does not match** the $\beta_{schooling}=0.02667$ just stated (and matches the actual log-lin regression table above: 0.026674), and it is also missing the ×100 step when displaying the intermediate figure ($0.027$ instead of $2.7$). But the final conclusion "2.700 percent" once again matches exactly if $\beta_1=0.02667$ (not 0.297) is used. The same "0.297" figure appearing repeatedly in both places (log-log and log-lin) further supports the possibility that this is leftover data from an earlier version of the slide/dataset, rather than two independent random errors. Noted here, without silently correcting the source.</span>

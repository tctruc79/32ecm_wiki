...
**Các mô hình/ước lượng khả dụng theo slide**:
<br><span class="en">**Models/estimators available per the slide**:</span>
- **RE-IV** (Generalized 2SLS) — slide ghi rõ nguyên văn "**not covered**". Đây là gap được chính slide **xác nhận minh bạch** (khác các gap phải tự suy luận từ sự vắng mặt nội dung) — Course Outline có liệt kê "2SLS RE estimator"/"G2GLS estimator" cho Topic 13, nhưng bản slide thực tế không dạy. Ghi nhận nguyên văn.
<br><span class="en">**RE-IV** (Generalized 2SLS) — the slide states verbatim "**not covered**". This is a gap **explicitly confirmed** by the slide itself (unlike gaps that must be inferred from the absence of content) — the Course Outline lists "2SLS RE estimator"/"G2GLS estimator" for Topic 13, but the actual slide does not teach it. Recorded verbatim.</span>
...
Slide có một mục riêng "Two-way FE-IV Regression" — về nội dung, đây chỉ là thêm **time fixed effects** $\gamma_t$ vào bên cạnh $\alpha_i$ (giống two-way FE ở [[concepts/fixed-random-effects-model]]), khai báo trong `fixest` bằng `| id + year |` thay vì `| id |`:
<br><span class="en">The slide has a dedicated section "Two-way FE-IV Regression" — in substance, this is just adding **time fixed effects** $\gamma_t$ alongside $\alpha_i$ (like two-way FE in [[concepts/fixed-random-effects-model]]), declared in `fixest` with `| id + year |` instead of `| id |`:</span>
...
Slide chỉ trình bày đoạn code cho cả 5 loại SE (homoskedastic, individual hetero, clustered by `id`, two-way clustered `id+year`, Driscoll-Kraay) — **không kèm bảng kết quả số** cho phần này, khác với FD-IV/FE-IV một chiều ở trên vốn có bảng số đầy đủ. Ghi nhận đúng như slide trình bày, không suy diễn số liệu.
<br><span class="en">The slide only presents the code for all 5 types of SE (homoskedastic, individual hetero, clustered by `id`, two-way clustered `id+year`, Driscoll-Kraay) — **without an accompanying numeric results table** for this part, unlike the one-way FD-IV/FE-IV above which have full numeric tables. Recorded exactly as presented in the slide, without inferring numbers.</span>
...
> **Lưu ý về nguồn**: bản slide **không đưa ra quy tắc quyết định tường minh** giữa FD-IV và FE-IV ngoài việc trình bày cả hai như hai lựa chọn song song (chỉ khác nhau ở SE robustness notes, vốn giống hệt nhau giữa hai mô hình). Phần lập luận "khi nào chọn" dưới đây là **kiến thức nền tổng quát** của econometrics panel data (không phải trích từ slide này) — nêu ra vì hữu ích cho việc chọn ước lượng trong thực hành/luận văn, nhưng cần phân biệt rõ đây không phải nội dung được slide xác nhận trực tiếp.
> <br><span class="en">**Note on the source**: the slide **does not give an explicit decision rule** between FD-IV and FE-IV beyond presenting both as parallel options (differing only in SE robustness notes, which are identical across the two models). The "which to choose" reasoning below is **general panel-data econometrics background knowledge** (not excerpted from this slide) — included because it's useful for choosing an estimator in practice/thesis work, but it must be clearly distinguished as content not directly confirmed by the slide.</span>
...
**Lưu ý thực hành riêng cho panel** (khác cross-section, ghi rõ trong slide): two-way clustered SE **không đáng tin cậy** khi $T$ nhỏ; Driscoll-Kraay SE **không khả dụng** cho ước lượng LIML/Fuller trong các package R thông dụng — đây là một ràng buộc thực hành cần nhớ khi chọn loại SE, không chỉ chọn theo "loại robust nhất luôn tốt nhất".
<br><span class="en">**Panel-specific practical note** (unlike cross-section, explicitly stated in the slide): two-way clustered SE is **not reliable** when $T$ is small; Driscoll-Kraay SE is **not available** for LIML/Fuller estimation in common R packages — this is a practical constraint to remember when choosing an SE type, not simply picking "the most robust type is always best."</span>

**Về GMM trong mô hình tĩnh**: với đặc tả tĩnh (static, không có biến trễ), IV-GMM **không mở rộng đáng kể** bộ instrument so với 2SLS — cả hai dựa trên cùng excluded instruments, chỉ khác nhau ở ma trận trọng số $W$ (2SLS dùng $W=(Z'Z)^{-1}$; GMM hiệu quả dùng $W=S^{-1}$). Dưới homoskedasticity, GMM hiệu quả **thu gọn về đúng 2SLS**; dưới heteroskedasticity, GMM gán trọng số thấp hơn cho quan sát có phương sai sai số lớn nên hiệu quả hơn — nhưng **lợi ích thực nghiệm trong mô hình tĩnh khá hạn chế** theo ghi chú của slide. (So sánh: GMM phát huy tác dụng rõ rệt hơn nhiều trong **dynamic panel** — xem [[concepts/dynamic-panel-data-models]] — nơi bộ instrument nội tại mở rộng đáng kể theo số lag khả dụng.)
...
Tái sử dụng đúng bốn nhóm kiểm định từ [[concepts/endogeneity-iv-regression]], điều chỉnh cho panel. Slide lưu ý: "*In R, diagnostic tests for IV regression with panel data is limited*" — công cụ sẵn có trong R hạn chế hơn so với cross-section thuần túy.
<br><span class="en">Reuses exactly the four groups of tests from [[concepts/endogeneity-iv-regression]], adjusted for panel. The slide notes: "*In R, diagnostic tests for IV regression with panel data is limited*" — the tools available in R are more limited than for pure cross-section.</span>
...
> **Lưu ý quan trọng nhất của mục này — bẫy thi hay gặp**: **KP-F không so sánh trực tiếp được với ngưỡng Stock-Yogo**, vì SY được xây dựng dưới giả định homoskedastic còn KP-F thì không — dùng sai một tiêu chí cho một thống kê không tương thích là lỗi kỹ thuật. Slide ghi rõ: trong thực hành, người ta vẫn **so sánh không chính thức (informally)** KP-F với SY hoặc với ngưỡng 10, dù biết đây không phải phép so sánh chặt chẽ về mặt lý thuyết — chỉ là quy ước thực hành phổ biến, không phải một kiểm định chính danh.
...
**Ghi chú về ví dụ số**: ví dụ minh họa của slide (bảng ở mục 7.1) chỉ trình bày **một** giá trị F (dùng two-way clustered SE) và xác nhận bằng lời rằng với 1 biến nội sinh, F này chính là CD-F — slide **không đưa thêm một con số KP-F tách biệt** để đối chiếu trong đúng ví dụ số này. Ghi nhận đúng những gì slide có, không suy diễn thêm một giá trị KP-F không được cho.
<br><span class="en">**Note on the numeric example**: the slide's illustrative example (table in section 7.1) presents only **one** F value (using two-way clustered SE) and confirms in words that with 1 endogenous variable, this F is exactly CD-F — the slide **does not provide a separate KP-F number** to compare within this exact numeric example. Recorded exactly as the slide has it, without inferring an unprovided KP-F value.</span>
...
**Điểm nhấn mạnh nhất của slide, đáng ghi nguyên văn tinh thần**: dù kết quả Sargan "ủng hộ" validity, slide tự cảnh báo hai lớp giới hạn:
<br><span class="en">**The point the slide emphasizes most, worth recording in spirit verbatim**: even though the Sargan result "supports" validity, the slide itself warns of two layers of limitation:</span>
...
7. Không bác bỏ Sargan/Hansen J rồi coi là "đã chứng minh instrument hợp lệ" — và **quan trọng hơn**: quên rằng độ tin cậy cuối cùng của instrument luôn dựa vào **lập luận kinh tế** (exclusion restriction hợp lý), không phải bản thân con số kiểm định — đây là điểm slide tự nhấn mạnh hai lần, không phải suy diễn của người viết wiki.
<br><span class="en">Failing to reject Sargan/Hansen J and then treating it as "proof the instrument is valid" — and **more importantly**: forgetting that an instrument's ultimate credibility always rests on **economic reasoning** (a plausible exclusion restriction), not the test statistic itself — a point the slide itself emphasizes twice, not an inference by the wiki author.</span>

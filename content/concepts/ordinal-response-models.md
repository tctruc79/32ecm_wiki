...
> **Lưu ý nhỏ về nguồn**: slide, ở trang phát biểu quy tắc lần đầu (trang "MODELLING ORDINAL RESPONSE"), viết bất đẳng thức **nghiêm ngặt** ("$y=0$ if $y^*<u_1$"; "$y=2$ if $y^*>u_2$"), nhưng ở trang suy ra công thức xác suất ngay sau đó lại dùng dấu **"≤"/"≥"** ("$Pr(y=0)=Pr(y^*\le u_1)$"). Đây là một khác biệt ký hiệu nhỏ giữa hai trang liền kề của cùng bộ slide — về mặt toán học không ảnh hưởng kết quả (vì $y^*$ liên tục nên $Pr(y^*=u_1)=0$), nhưng ghi chú lại ở đây thay vì âm thầm chọn một quy ước, theo đúng nguyên tắc của wiki này. Trang này dùng quy ước "≤/>" theo đúng trang suy luận xác suất, vì đó là trang được dùng để tính toán trong phần còn lại của slide.
> <br><span class="en">**A small note on the source**: the slide, on the page that first states the rule (the "MODELLING ORDINAL RESPONSE" page), writes **strict** inequalities ("$y=0$ if $y^*<u_1$"; "$y=2$ if $y^*>u_2$"), but the page immediately after, which derives the probability formulas, uses **"≤"/"≥"** ("$Pr(y=0)=Pr(y^*\le u_1)$"). This is a small notational inconsistency between two adjacent pages of the same slide deck — mathematically it doesn't affect the result (since $y^*$ is continuous, $Pr(y^*=u_1)=0$), but it is noted here rather than silently picking one convention, following this wiki's own principle. This page uses the "≤/>" convention matching the probability-derivation page, since that is the page used for computation in the rest of the slides.</span>
...
> **Ghi chú về nguồn**: slide đặt tiêu đề của trang barplot phân phối biến phụ thuộc là **"DEPENDENT VARIABLE: BLOOD PRESSURE"**, dù nội dung trang đó (và toàn bộ phần còn lại của deck) chỉ dùng biến `eatout`, không có biến "huyết áp" nào trong dữ liệu. Đây nhiều khả năng là lỗi copy-paste từ một slide/deck khác (ghi chú này từng được để lại trong log ingest gốc của wiki) — giữ nguyên ghi chú, không âm thầm sửa tiêu đề.
> <br><span class="en">**Note on the source**: the slide titles the dependent-variable distribution barplot page **"DEPENDENT VARIABLE: BLOOD PRESSURE,"** even though that page's content (and the rest of the deck) only uses the `eatout` variable — there is no "blood pressure" variable in the data. This is most likely a copy-paste error from a different slide/deck (this note was already left in the wiki's original ingest log) — the note is kept as-is, the title is not silently corrected.</span>
